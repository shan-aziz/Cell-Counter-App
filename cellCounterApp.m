function cellCounterApp
    ...
    uibutton(fig, 'push', 'Text', 'Load Image', 'Position', [20, 50, 100, 30], 'ButtonPushedFcn', @(btn,event) loadImage(ax));

    % Button to enhance the image
    uibutton(fig, 'push', 'Text', 'Enhance Image', 'Position', [140, 50, 100, 30], 'ButtonPushedFcn', @(btn,event) enhanceImage(ax));

    % Button to segment the image
    uibutton(fig, 'push', 'Text', 'Segment Image', 'Position', [260, 50, 100, 30], 'ButtonPushedFcn', @(btn,event) segmentImage(ax, sizeThresholdField));

    % Button to count the cells
    uibutton(fig, 'push', 'Text', 'Count Cells', 'Position', [380, 50, 100, 30], 'ButtonPushedFcn', @(btn,event) countCells(ax));
